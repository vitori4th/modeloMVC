...
class LimiteConsultaTurmas():
    def __init__(self,controle, janela):
        self.controle = controle
        self.janela = janela
        self.my_i = simpledialog.askstring("Consulta","Digite uma disciplina",parent=self.janela)
...
class CtrlTurma():       
    ...
    def consultaTurmas(self, janela):
        self.jan = janela
        LimiteConsultaTurmas(self, self.jan )
    ...
    def searchHandler(self, nr,limite):
        ...
        if nr == None:
            ...
            if nr == "":
                ...
                for a in self.listaTurmas :  
                    if a.getDisciplina().getCodigo() == nr:
                        print(a.getDisciplina())
                        str += '------\n'
                        str += 'Código: ' + a.getCodigo() + '\n'
                        str += 'Disciplina: ' + a.getDisciplina().getCodigo() + '\n'
                        str += 'Estudantes:\n'
                        for estud in a.getEstudantes():
                            str += estud.getNroMatric() + ' - ' + estud.getNome() + '\n'
                        str += '------\n'
                    
                if str == '':
                    self.limiteCon.mostraJanela('Fracasso',"Nenhuma turma encontrada") 
                else:
                    self.limiteLista = LimiteMostraTurmas(str)
